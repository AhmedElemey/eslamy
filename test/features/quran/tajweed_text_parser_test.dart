...
void main() {
  group('parseTajweedText', () {
    ...
    test(
      'parses Al-Fatiha 1:1 (real API response) into the correct rule sequence',
      () {
        ...
        final hamzaSegment = segments.firstWhere(
          (s) => s.rule?.code == 'h' && s.text == 'ٱ',
        );
    ...
    test(
      'parses Al-Fatiha 1:3, which starts with a rule tag (no leading plain text)',
      () {
        ...
        expect(segments.map((s) => s.text).join(), 'ٱلرَّحْمَـٰنِ ٱلرَّحِيمِ');
      },
    );
