...
class NotificationService {
  ...
  // Prevents the foreground minute-tick from re-firing an Adhan that was
  // already shown (via scheduleAdhan's same-minute path, or a previous tick)
  // during this process lifetime.
  final Set<String> _firedAdhanKeys = <String>{};
  ...
  Future<void> scheduleAdhan({
    ...
    for (final day in [today, tomorrow]) {
      for (final name in _adhanPrayerNames) {
        ...
        switch (adhanDeliveryFor(time, now)) {
          case AdhanDelivery.skip:
            continue;
          case AdhanDelivery.showNow:
            // Same clock minute as the prayer — zonedSchedule would either
            // be in the past (plugin throws / OS drops it) or fire so late
            // in the minute that the user already thinks it missed. Show
            // immediately so "prayer time == now" actually notifies.
            final key = _adhanFiredKey(name, time);
            if (_firedAdhanKeys.contains(key)) continue;
            _firedAdhanKeys.add(key);
            await _plugin.show(
              notificationId,
              _adhanTitle(name, l10n),
              _adhanBody(name, l10n),
              details,
            );
            if (kDebugMode) {
              debugPrint('Adhan shown immediately for $name (current minute)');
            }
          case AdhanDelivery.schedule:
            ...
            if (kDebugMode) {
              debugPrint('Adhan scheduled for $name at $scheduled');
            }
        }
      }
    }
  ...
  /// If any of [today]'s prayers is in the current clock minute and has not
  /// already been shown this process, posts that Adhan notification now.
  /// Used while the app is open so a prayer boundary is not missed just
  /// because the OS scheduled alert was cancelled, delayed, or skipped.
  Future<void> notifyIfPrayerTimeNow({
    required Map<String, DateTime> today,
    AppLocalizations? l10n,
  }) async {
    if (!_initialized) {
    ...
    for (final name in _adhanPrayerNames) {
      final time = today[name];
      if (time == null) continue;
      if (adhanDeliveryFor(time, now) != AdhanDelivery.showNow) continue;
      final key = _adhanFiredKey(name, time);
      final id = _adhanBaseId + _adhanPrayerNames.indexOf(name);
      if (_firedAdhanKeys.contains(key) || activeIds.contains(id)) {
        _firedAdhanKeys.add(key);
        continue;
      }
      _firedAdhanKeys.add(key);
      ...
      if (kDebugMode) {
        debugPrint('Adhan shown for $name (prayer time == current minute)');
      }
    }
  }
  ...
  String _adhanFiredKey(String name, DateTime time) =>
      '$name-${time.year}-${time.month}-${time.day}';

  String _adhanTitle(String name, AppLocalizations? l10n) {
    final displayName = l10n == null ? name : _localizedPrayerName(l10n, name);
    return l10n?.adhanNotificationTitle(displayName) ?? 'Adhan — $name';
  }

  String _adhanBody(String name, AppLocalizations? l10n) {
    final displayName = l10n == null ? name : _localizedPrayerName(l10n, name);
    return l10n?.adhanNotificationBody(displayName) ??
